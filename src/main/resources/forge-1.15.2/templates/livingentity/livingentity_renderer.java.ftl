...
public class ${name}Renderer extends <#if humanoid>Biped<#else>Mob</#if>Renderer<${name}Entity, ${model}> {
	...
	@Override protected void applyRotations(${name}Entity entity, MatrixStack poseStack, float ageInTicks, float rotationYaw, float partialTicks) {
        ...
        if(<@procedureOBJToConditionCode data.isShakingCondition/>)
		    rotationYaw += (float) (Math.cos((double) entity.ticksExisted * 3.25D) * Math.PI * 0.25D);
