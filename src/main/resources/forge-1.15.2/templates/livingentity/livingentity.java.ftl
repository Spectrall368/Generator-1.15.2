...
public class ${name}Entity extends ${extendsClass}Entity <#if interfaces?size gt 0>implements ${interfaces?join(",")}</#if> {
	...
	@Override public boolean processInteract(PlayerEntity sourceentity, Hand hand) {
		...
		ActionResultType retval = ActionResult.resultSuccess(this.world.isRemote()).getType();
		...
		<#if (data.tameable && data.breedable)>
			...
			if (itemstack.getItem() instanceof SpawnEggItem) {
				retval = ActionResult.resultSuccess(super.processInteract(sourceentity, hand)).getType();
			} else if (this.world.isRemote()) {
				retval = (this.isTamed() && this.isOwner(sourceentity) || this.isBreedingItem(itemstack))
						? ActionResult.resultSuccess(this.world.isRemote()).getType() : ActionResultType.PASS;
			} else {
				if (this.isTamed()) {
					if (this.isOwner(sourceentity)) {
						if (item.isFood() && this.isBreedingItem(itemstack) && this.getHealth() < this.getMaxHealth()) {
							...
							retval = ActionResult.resultSuccess(this.world.isRemote()).getType();
