	...
	</#if>
{
	...
	<#macro blockProperties>
	    ...
	    REDSTONE_LIGHT
	    <#else>
	    ${blockSetType?replace("OAK", "WOOD")?replace("STONE", "ROCK")}
	    </#if>
    ...
    <#if data.blockBase?has_content && data.blockBase == "Leaves">
   	@Override public boolean causesSuffocation(BlockState state, IBlockReader world, BlockPos pos) {
		return false;
   	}

   	@OnlyIn(Dist.CLIENT)
   	@Override public boolean isViewBlocking(BlockState state, IBlockReader world, BlockPos pos) {
		return false;
   	}
	</#if>
